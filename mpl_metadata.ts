...
async function main(){
    ...
    const myKeypair = loadWalletKey("AndyUCWqhEnEMqHAByoRSHz2mvQxdyXyki9UQ7YCrTBY.json"); //YOUR WALLET OR CREATE ONE
    const mint = new web3.PublicKey("SDTHBG48VGNoGS1U2ArnvMUZ3dxyGr1F4TT1ojD4QDB"); // WALLET MINT OR CREATE ONE

    const umi = createUmi("https://api.devnet.solana.com"); // for the devnet leave this or mainnet creates an endpoint
    const signer = createSignerFromKeypair(umi, fromWeb3JsKeypair(myKeypair))
    umi.use(signerIdentity(signer, true))

    const ourMetadata = { // TODO change those values!
        name: "Name Token", // NAME TOKEN
        symbol: "Simbol", //SIMBOL TOKEN
        uri: "https://URI-IPFS-METADATA.json", //URI-IPFS-METADATA.JSON
    }
    const onChainData = {
        ...ourMetadata,
        // we don't need that
